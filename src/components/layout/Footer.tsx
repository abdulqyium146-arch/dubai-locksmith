...
const FOOTER_CAR_BRANDS = carBrands.slice(0, 10).map((b) => ({
  label: b.name,
      ...
      <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          ...
          <div>
            ...
            <ul className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2" role="list">
              {FOOTER_CAR_BRANDS.map(({ label, href }) => (
                <li key={href}>
                  <Link
                    href={href}
                    className="text-xs text-white/70 transition-colors hover:text-brand-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded leading-snug block py-1.5"
          ...
          {/* Column 5 — Company & Google Rating ─────────────────────────────── */}
          <div>
            <h3 className="font-heading text-sm font-semibold uppercase tracking-widest text-brand-gold">
              Company
            </h3>
            ...
            <a
              ...
              aria-label={`${GOOGLE_RATING}★ Google Rated — View our reviews`}
            >
